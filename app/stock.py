...
class Stock:
    ...
    def find_close_local_maxima(self, original_close_data: pd.DataFrame, smoothed_data: pd.DataFrame, interval: int=5) -> pd.DataFrame:
        # Find local maxima from smoothed data
        peak_indexs = argrelextrema(smoothed_data["Data"].to_numpy(), np.greater)[0]

        peaks = original_close_data.iloc[peak_indexs]["Close"]
    ...
    def find_close_local_minima(self, original_close_data: pd.DataFrame, smoothed_data: pd.DataFrame, interval: int=5) -> pd.DataFrame:
    ...
    def find_close_extrema(self, original_close_data: pd.DataFrame, smoothed_data: pd.DataFrame, interval: int=5) -> pd.DataFrame:
        peaks = self.find_close_local_maxima(original_close_data, smoothed_data, interval)
        bottoms = self.find_close_local_minima(original_close_data, smoothed_data, interval)
    ...
    def list_close_stock_peak_bottom_info(self):
        ...
        peaks_and_bottoms = self.find_close_extrema(stock_close_data, smoothed_close_data, interval=self.find_extrema_interval)
